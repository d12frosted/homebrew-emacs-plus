...
#+begin_src bash
  # Homebrew 6.0+ does not load non-official taps until you trust them, and
  # `brew tap` checks every formula and cask before accepting a tap, so trust
  # first. See https://docs.brew.sh/Tap-Trust.
