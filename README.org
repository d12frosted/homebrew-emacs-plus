...
** Table of Contents :TOC_3:
  ...
  - [[#features-explained][Features explained]]
    ...
    - [[#gccemacs][gccemacs]]
  - [[#icons][Icons]]
  - [[#spotlight-alias][Spotlight Alias]]
...
| =--with-no-titlebar=                              | =--with-no-titlebar-and-round-corners=           |
|---------------------------------------------------+--------------------------------------------------|
...
Round corners are currently not available. See [[https://github.com/d12frosted/homebrew-emacs-plus/issues/320][→ #320]] for progress.
...
** Spotlight Alias
Symlinking (via ~ln -s~) ~Emacs.app~ won't be visible in Spotlight. However, you can create an Alias through Finder. Note: I don't know if this will persist through updates.
Source: https://apple.stackexchange.com/a/32495/449886
1. Goto ~emacs-plus~ install location in Finder (run ~brew info emacs-plus~ to find install directory)
2. Right-click ~Emacs.app~ -> ~Create Alias~
3. Move the ~Emacs alias~ into ~/Applications~ and rename it as you wish.

It might take a few seconds for macOS to index the file. You should now be able to launch Emacs from Spotlight (no reboot was needed for me).
Note: If it doesn't work for you, there is a comments thread in the SO post that talks about potential workarounds.

Alternatively, you can run this script. Change <version> to your installed emacs version.
#+begin_src terminal
osascript -e 'tell application "Finder" to make alias file to posix file "/opt/homebrew/opt/emacs-plus@<version>/Emacs.app" at POSIX file "/Applications"'
