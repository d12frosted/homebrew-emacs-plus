...
** Spotlight Alias
Symlinking (via ~ln -s~) ~Emacs.app~ won't be visible in Spotlight. However, you can create an Alias through Finder. 
*** Apple Script
Change <version> to your installed emacs version.
#+begin_src terminal
...
*** Manual
Note: I don't know if this will persist through updates.
...
It might take a few seconds for macOS to index the file. You should now be able to launch Emacs from Spotlight (no reboot needed).
