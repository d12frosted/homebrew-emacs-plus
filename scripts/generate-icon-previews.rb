...
class IconPreviewGenerator
  ...
  # Render a person reference (author or maintainer) as a Markdown link or
  # plain name. Returns nil when no usable information is present.
  def person_link(person)
    return nil if person.nil? || person.empty?

    if person['github']
      "[#{person['github']}](https://github.com/#{person['github']})"
    elsif person['name'] && person['url']
  ...
  # Author column: prefer the original `author`, falling back to `maintainer`
  # for icons that predate the split. When both are present and differ, the
  # maintainer is credited alongside the author.
  def format_author(meta)
    author = person_link(meta['author'])
    maintainer = person_link(meta['maintainer'])

    return maintainer || 'Unknown' if author.nil?
    return author if maintainer.nil? || maintainer == author

    "#{author} (maintained by #{maintainer})"
  end
  ...
  def generate_readme
    ...
    content = <<~HEADER
      <!--
        ...
        It is generated by scripts/generate-icon-previews.rb from the per-icon
        metadata.json files. Any manual edits will be overwritten on the next run.
        To change content, edit the script or an icon's metadata.json and re-run:
    ...
    content += <<~FOOTER
      ...
      - `metadata.json` - Icon metadata (see fields below)
      - `preview.png` - 128x128 preview image

      **Metadata fields:**
      - `name` - Icon identifier (matches the directory name)
      - `description` - Short description of the icon
      - `author` - Original artwork creator: `{ "name": "...", "url": "..." }`
        or `{ "github": "..." }`
      - `maintainer` - Who packages/adapts the icon for Emacs+: `{ "github": "..." }`
        or `{ "name": "..." }`. When it differs from `author`, both are credited.
        For backwards compatibility, `maintainer` is used as the author when no
        explicit `author` is set.
      - `homepage` - Source URL shown in the "Source" column
      - `license` - SPDX license identifier of the artwork (e.g. `GPL-2.0`)
      ...
      **Metadata fields for Tahoe icons:**
      ...
      - `pre_tahoe` - `true` if the `.icns` fallback is a faithful match of the icon on
        pre-Tahoe macOS, `false` otherwise. Shown in the "Pre-Tahoe" column.
