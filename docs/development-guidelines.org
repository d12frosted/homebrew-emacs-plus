...
#+begin_src bash
  $ make formula-30              # Build Emacs 30
  $ make formula-31              # Build Emacs 31
  $ make formula-31 --HEAD       # Build from HEAD
#+end_src

This creates a temporary =emacs-plus-local= formula and installs it. That way you can play with formula without uninstalling =emacs-plus=.
...
#+begin_src bash
  ...
  $ make postinstall-cask        # Simulate cask postflight
#+end_src
