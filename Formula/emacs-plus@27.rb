...
class EmacsPlusAT27 < EmacsBase
  ...
  # Post-install setup: info manuals and re-signing. post_install_steps only
  # takes literal steps, so the Ruby in Library/ runs through
  ...
  post_install_steps do
    run "emacs-plus-postinstall",
        base:         :libexec,
        args:         ["--prefix", "{{prefix}}", "--install-info"],
        print_stdout: true
  end
